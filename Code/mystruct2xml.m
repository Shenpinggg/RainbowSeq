...
% Val2str subfunction bugfix by H. Gsenger, 19-9-2011
    ...
    xmlname_sc = strrep(xmlname_sc, 'u_', '_');
    ...
    if(nargout == 0)
        %save xml file
        myXMLwrite(file,docNode);
    else
        varargout{1} = myXMLwrite(docNode);
    end  
end
...
function [] = parseStruct(s,docNode,curNode,pName)
    ...
    for i = 1:length(fnames)
        ...
        curfield_sc = strrep(curfield_sc, 'u_', '_');
        ...
        if (strcmp(curfield,'Attributes'))
            ...
            if (isstruct(s.(curfield)))
                ...
                for a = 1:length(attr_names)
                    ...
                    cur_attr_sc = strrep(cur_attr_sc,'u_','_');
